		...
		init:function() {
			if ($("#"+this.view.map).is(":visible")) {
				...
				try {
					...
					this.model.coords = new L.LatLng(localConfig.defaultLat, localConfig.defaultLng);
					this.marker = undefined;
				};
				this.map = L.map(this.view.map,{
					center: this.model.coords,
					zoom: localConfig.defaultZoom
				})
				this.marker && (this.marker.addTo(this.map));
				this.tiles = new L.tileLayer(localConfig.tileUrl, {
				    attribution: localConfig.tileAttribution
				})
